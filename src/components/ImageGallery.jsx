...
const ImageGallery = ({ isOpen, onClose, onSelectImage, selectedUrl, embedded = false }) => {
    ...
    const content = (
        <div className={embedded ? '' : 'bg-[#1a1a1a] border border-white/20 rounded-2xl p-8 max-w-6xl w-full max-h-[90vh] overflow-y-auto'}>
            {/* Header */}
            <div className="flex items-center justify-between mb-6">
                <h2 className={`${embedded ? 'text-xl' : 'text-2xl'} font-bold text-white flex items-center gap-2`}>
                    <ImageIcon className="w-6 h-6 text-primary" />
                    {embedded ? 'Galería' : 'Galería de Imágenes'}
                </h2>
    ...
    // Otherwise, return content wrapped in modal
    return (
        <AnimatePresence>
            <motion.div
                initial={{ opacity: 0 }}
